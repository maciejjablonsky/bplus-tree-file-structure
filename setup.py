...
parser.add_argument(
    '--run-cmake', help='Runs cmake command to setup build directory', action='store_true')
parser.add_argument(
    '--clean', help='Deletes build directory before running any other command', action='store_true')
parser.add_argument(
...
parser.add_argument('--build-release',
                    help='Build release target', action='store_true')
parser.add_argument(
...
build_dir_path = repository_path + \
    '/../build-{}'.format(os.path.basename(repository_path))
...
if args.open_code:
    subprocess.run(['code', repository_path],
